...
class HeuristicGiver(Giver):
    ...
    def __init__(self,
        ...
        # self.board = board
        super().__init__(board.tolist(), allIDs)
    ...
    def get_next_clue(self, game_state: List[int],
                      score: int):
        available_targets = [word for word in self.pos_words if
                             game_state[self.board.index(word)] == UNREVEALED]
        clues_by_group = []
        ...
        for count in range(num_words, 0, -1):
            for group in combinations(range(num_words), count):
                ...
                clues_for_group = self._get_clues(target_group, self.neg_words)
                if clues_for_group is not None:
                    clues_by_group.append(self._get_clues(target_group, self.neg_words))
